...
def main(eventNumber,rankType,areacode,basePath,JsonPath):
    ...
    def basicGet(eventNumber,rankType,areacode):
        ...
        eventName=eventDict['eventName'][areacode] 
        eventStart=int(eventDict['startAt'][areacode])/1000 #Convert the timestamp(unit:s)
        eventEnd=int(eventDict['endAt'][areacode])/1000
        try:
            #这里获取bannerTitle（临时）
            pic_filename=os.path.join(imgPath,f'{str(eventNumber)}title.png')
    ...
    def noptHandle(eventName,basePath,rankType=2):
        im = Image.open(os.path.join(imgPath,"noPt.png"))
        im=im.convert('RGBA')
        txt=Image.new('RGBA', im.size, (0,0,0,0))
        fnt=ImageFont.truetype(os.path.join(imgPath,"HYZhengYuan-55W.ttf"), 50)
        fnt2=ImageFont.truetype(os.path.join(imgPath,"HYZhengYuan-55W.ttf"), 20)
        title='《'+str(eventName)+'》'
    ...
    def graphDraw(pctList,ptList,slpList,ufinList,finList,bfinList,predPct):    
        ...
        plt.savefig(os.path.join(imgPath,'chart.png'), dpi=600, bbox_inches='tight')
        plt.close()  
        
        fig, ax = plt.subplots()
        fig.patch.set_alpha(0.) 
        plt.pie([100-progress,progress],wedgeprops=dict(width=0.1))
        fig.savefig(os.path.join(imgPath,'ring.png'),dpi=450, transparent=True, bbox_inches='tight')
        plt.close() 
        ...
        if  len(pctList)>=5 and pctList[-3]>=90:
            ...
            plt.savefig(os.path.join(imgPath, 'Detailchart.png'), dpi=600, bbox_inches='tight')
            plt.close()  
            able=1
        else:
            able=0   
    ...
    def picDraw(pctList,ptList,slpList,ufinList,finList,bfinList,eventName,eventStart,eventEnd,rankType,able,basePath):
        ...
        im = Image.open(os.path.join(imgPath,"backgroundNEO.png")).convert("RGBA")  
        
        box=(0,880,3500,3180)
        chart1= Image.open(os.path.join(imgPath,'chart.png')).convert("RGBA")  
        region = chart1
        region = region.resize((box[2] - box[0], box[3] - box[1]))
        im.paste(region, box)
        if able==1:
            box2= (0,880,1680,2070)
            chart2= Image.open(os.path.join(imgPath,'Detailchart.png')).convert("RGBA")  
            region2 = chart2
        ...
        chart3= Image.open(os.path.join(imgPath,'ring.png')).convert("RGBA")  
        region3 = chart3
        ...
        titlepic=Image.open(os.path.join(imgPath, f'{str(eventNumber)}title.png')).convert("RGBA")  
        region4=titlepic
        ...
        fnt=ImageFont.truetype(os.path.join(imgPath, "HYZhengYuan-55W.ttf"), 160)
        fnt2=ImageFont.truetype(os.path.join(imgPath, "Helvetica Bold.ttf"), 165) 
        fnt3=ImageFont.truetype(os.path.join(imgPath, "HYZhengYuan-55W.ttf"), 90)
...
def GetDataPic(areacode,basePath,JsonPath,PredNow=True,Benum=0,RankType=-1):
